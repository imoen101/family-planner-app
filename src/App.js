...
function countCreditsFromBaseline(slots, baselineSlots) {
  ...
  for (const key of Object.keys(slots)) {
    ...
    if (current.slotType !== "Swappable" || baseline.slotType !== "Swappable") {
      continue;
    }
...
function getSlotBackground(state, slotType) {
  const colors = getSlotColors(state);

  if (slotType === "Swappable") {
    return {
      backgroundImage: `repeating-linear-gradient(135deg, ${colors.background} 0px, ${colors.background} 10px, rgba(255,255,255,0.45) 10px, rgba(255,255,255,0.45) 18px)`,
...
async function pullSharedPlanner() {
  const url = `${SUPABASE_URL}/rest/v1/planner_state?id=eq.${encodeURIComponent(SHARED_PLANNER_ID)}&select=data`;
...
export default function App() {
  ...
  const selectedSlot = data.slots[data.selectedKey];
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        ...
        >
          <div style={{ ...miniCardStyle(), padding: 20 }}>
            <div
              style={{
                display: "flex",
            ...
            <div style={{ marginBottom: 16, color: syncMessage ? "#0f172a" : "#64748b", fontSize: 13 }}>
            ...
            >
              <div style={miniCardStyle()}>
                ...
                <div style={{ fontSize: 28, fontWeight: 700, color: "#0369a1", marginTop: 4 }}>{summary.imogenDuty}</div>
              </div>
              <div style={miniCardStyle()}>
                <div style={{ fontSize: 13, color: "#64748b" }}>Dodo on duty</div>
                <div style={{ fontSize: 28, fontWeight: 700, color: "#b45309", marginTop: 4 }}>{summary.dodoDuty}</div>
              </div>
              <div style={miniCardStyle()}>
                <div style={{ fontSize: 13, color: "#64748b" }}>Flexible</div>
                <div style={{ fontSize: 28, fontWeight: 700, color: "#15803d", marginTop: 4 }}>{summary.flexible}</div>
              </div>
              <div style={miniCardStyle()}>
                <div style={{ fontSize: 13, color: "#64748b" }}>Fixed slots</div>
                <div style={{ fontSize: 28, fontWeight: 700, marginTop: 4 }}>{summary.fixed}</div>
              </div>
            </div>

            <div style={{ display: "flex", gap: 16, flexWrap: "wrap", fontSize: 13, color: "#475569", marginBottom: 16 }}>
            ...
            <div style={{ overflowX: "auto" }}>
              ...
              >
                ...
                >
                  ...
                  {DAYS.map((day) => (
                    <div key={day} style={{ padding: 12, textAlign: "center" }}>
                      {day}
                    </div>
                  ))}
                </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <div style={{ marginTop: 8, fontSize: 11, color: "#475569" }}>{slot.slotType}</div>
                          </div>
                        </button>
                      );
                    })}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ ...miniCardStyle(), padding: 20 }}>
              <h2 style={{ marginTop: 0, marginBottom: 16, fontSize: 20 }}>Selected slot</h2>
              ...
              <div style={{ marginBottom: 18 }}>
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  {STATES.map((state) => (
                    <button
                      key={state}
                      style={buttonStyle(selectedSlot.state === state)}
              ...
              <div style={{ marginBottom: 18 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  {SLOT_TYPES.map((slotType) => (
                    <button
                      key={slotType}
                      style={buttonStyle(selectedSlot.slotType === slotType)}
            ...
            <div style={{ ...miniCardStyle(), padding: 20 }}>
              <h2 style={{ marginTop: 0, marginBottom: 16, fontSize: 20 }}>Free shifts</h2>

              <div style={{ display: "grid", gap: 12 }}>
                <div style={{ ...miniCardStyle(), padding: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ fontWeight: 700 }}>Imogen</div>
                      <div style={{ fontSize: 13, color: "#64748b" }}>Extra swappable duty since baseline</div>
                    </div>
                    <div style={{ fontSize: 28, fontWeight: 700, color: "#0369a1" }}>{creditSummary.imogen}</div>
                  </div>
                </div>
                ...
                <div style={{ ...miniCardStyle(), padding: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ fontWeight: 700 }}>Dodo</div>
                      <div style={{ fontSize: 13, color: "#64748b" }}>Extra swappable duty since baseline</div>
                    </div>
                    <div style={{ fontSize: 28, fontWeight: 700, color: "#b45309" }}>{creditSummary.dodo}</div>
                  </div>
                </div>
                ...
                >
                  {data.baselineSlots
                    ? `${creditSummary.changedSlots} slot${creditSummary.changedSlots === 1 ? "" : "s"} changed since baseline.`
                    : "Set a baseline first, then the planner will calculate free shifts automatically."}
                </div>
              </div>
            </div>

            <div style={{ ...miniCardStyle(), padding: 20 }}>
              <h2 style={{ marginTop: 0, marginBottom: 16, fontSize: 20 }}>How to use it</h2>
              <div style={{ display: "grid", gap: 8, fontSize: 14, color: "#475569" }}>
                <div>Click a slot to edit it on the right.</div>
                <div>Double-click a slot to quickly cycle through the states.</div>
                <div>Striped slots are swappable. Solid slots are fixed.</div>
                <div>Use “Set baseline” once your planned week is in place.</div>
                <div>Free shifts are calculated automatically from changes to swappable slots.</div>
                <div>Everything saves automatically in this browser.</div>
              </div>
            </div>
          </div>
        </div>
